...
from smoothing.filters import smooth_all_series
...
def run_demo(
    ...
    figure, axes = plt.subplots(8, 1, figsize=(14, 28), sharex=True)
    panels = [
    ...
    for axis, (method_name, title, color) in zip(axes, panels):
        ...
        axis.plot(series.frames, smoothed[method_name], label=title, color=color, linewidth=1.35)
